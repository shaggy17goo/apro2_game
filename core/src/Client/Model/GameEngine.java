...
import Client.Model.graphicalheros.Hero;
import Client.Model.map.Field;
import Client.Model.map.GameMap;
import Client.Model.map.Obstacle;
import Client.Model.map.Trap;
...
public class GameEngine {
    ...
    private static final List<Move> movesQueue = new ArrayList<>();//Queue<Move> movesQueue;
    private static final boolean readyToSend = false;
    ...
    private static void dfs(int y, int x, boolean[][] searched, int distance) {
        ...
        if (x - 1 >= 0 && distance > 0
                && ((gameMap.getFieldAt(y, x - 1).getObstacle() == null ||
                gameMap.getFieldAt(y, x - 1).getObstacle().isCrossable())))
            dfs(y, x - 1, searched, distance - 1);

        if (y + 1 < gameMap.getMaxY() && distance > 0
                && ((gameMap.getFieldAt(y + 1, x).getObstacle() == null ||
                gameMap.getFieldAt(y + 1, x).getObstacle().isCrossable())))
